...
class BinaryTreeNode(object):
    ...
    def height(self):
        ...
        Best and worst case running time: O(h) where h are traversing the height of the tree."""
        # check if ROOT Node exists
        if self is None:
            return 0
        # else:
        #     l_height = height(l_height)
        #     r_height = height(r_height)
        l_height = 0
        r_height = 0
...
class BinarySearchTree(object):
    ...
    def contains(self, item):
        ...
        Best case running time: O(1) if the root node holds the data we are looking for.
        Worst case running time: O(log(n)) if the node holding the correct data is the last one in the
                                    tree and the tree is ordered and balanced. 
        """
    ...
    def delete(self, item):
        ...
        #     new_node = BinaryTreeNode(self.find_inorder_parent(node).data)
    ...
    def find_inorder_parent(self, node):
    ...
    def _traverse_in_order_recursive(self, node, visit):
        ...
        Running time: O(n) where n is the number of nodes in the tree, and we are searching the
                        entire tree.
